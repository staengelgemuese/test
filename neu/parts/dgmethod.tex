...
\label{rkdgm}
	...
	\section{DG Space Discretization}
		...
		\begin{align}
			...
			\label{pde}
		\end{align}	
		for the concentration $c = c(\boldsymbol{x}, t)$  with $\vec{x} \in \Omega \subset \mathbb{R}^D$ and $t\in \mathbb{R}_0^+$ and a smooth function $\boldsymbol{f}:\mathbb{R} \rightarrow \mathbb{R}^D$ that also contains suitable initial and boundary conditions.\textcite{mueller2014} 
		...
		Some well-known examples of numerical fluxes contain \textcite{Cockburn1998}:
		\begin{itemize}
