...
\label{rkdgm}
	...
	\section{DG Space Discretization}
		...
		\begin{gather}	
			...
			\end{aligned}
		\end{gather}
		Considering that the cell's surface $\partial \mathcal{K}_i$ consists of internal or boundary edges $\left\{\mathcal{E}_{i,e}\right\}_{e = 1,...,E_i}$ we can rewrite \eqref{partialintegration} as
		\begin{align}
				\int\limits_{\mathcal{K}_i} \dfrac{\partial c}{\partial t}\Phi_{i,j} \, dV +
		...
		For we do not enforce continuity on $\mathcal{E}_{i,e}$ and thus \todo{$c_{n(i,e)}$ definieren}
		\begin{align}
		...
		\end{align}
		with the coefficient $C_R$ based on a local stability criterion. In this thesis we will use an estimate based on the maximum local wave speed \todo{cite Toro2009}
		\begin{align}
...
\end{table}		
	...
	Concerning the Euler equations the largest propagation velocity is given by $\underline{u} = ||\mathbf{u} || + a$ and by taking the influence of the approximation order $P$ into account we can use \todo{cite Cockburn and Shu 1991}
	\begin{align}
