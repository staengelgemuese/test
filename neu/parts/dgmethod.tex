\chapter{The Runge-Kutta Discontinous Galerkin Method}
	...
	In the following sections we will study the DG and RK methods separately considering simple examples and using the same notation as in \cite{Mueller.2014}.
	\section{DG space discretization}
		...
		\begin{align}
			...
			\label{pde}
		\end{align}	
		for the concentration $c = c(\boldsymbol{x}, t)$  with $\vec{x} \in \Omega \subset \mathbb{R}^D$ and $t\in \mathbb{R}_0^+$ and a smooth function $\boldsymbol{f}:\mathbb{R} \rightarrow \mathbb{R}^D$ that also contains suitable initial and boundary conditions.\cite{Mueller.2014} 
		...
		Some well-known examples of numerical fluxes contain \cite{Cockburn.2014}:
		\begin{itemize}
