...
\label{rkdgm}
	...
	\section{DG Space Discretization}
		...
		\begin{figure}[htp]
			...
			\begin{minipage}[b]{0.3\textwidth}
				\centering
				\includegraphics[height=3.3cm]{fvm.png}
			...
			\begin{minipage}[b]{0.3\textwidth}
				\centering
				\includegraphics[height=3.3cm]{dg.png}
		...
		Priorly to this we need a discretisation $\Omega_h$ of $\Omega$ consisting of a tesselation of cells $\left\{ \mathcal{K}_i \right\}_{i=1,...,N}$, where $h$ represents a measure for the size of the cells. Each cell $\mathcal{K}_i$ is of dimension $D$ with an outward unit normal vector $\boldsymbol{n}$. \\ \indent 
		...
		Therefore we will introduce a monotone, Lipschitz continuous numerical flux function \todo{erklären} $f = f(c^-, c^+, \mathbf{n}):\mathbb{R}^{D+2}\rightarrow\mathbb{R}$ satisfying the consistency property
		\begin{align}
		...
		\end{align}
		with the coefficient $C_R$ based on a local stability criterion. In this thesis we will use an estimate based on the maximum local wave speed that was developed by \cite{tororiemann}
		\begin{align}
...
\end{table}		
	...
	Concerning the Euler equations the largest propagation velocity is given by $\underline{u} = ||\mathbf{u} || + a$ and by taking the influence of the approximation order $P$ into account we can use the criterion developed by \cite{cockburn1991runge}
	\begin{align}
