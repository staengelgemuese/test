...
%\label{rkdgm}
	...
	The coefficients $\alpha \in \mathbb{R}^S$, $\beta \in \mathbb{R}^S$ and $\Gamma \in \mathbb{R}^S$ are specific for each RK method. Those of the most common RK methods are displayed in the Butcher Tableaus in \ref{RKtableaus}. They determine the stability and accuracy of the time integration scheme. \\
\begin{table}[h]
	\begin{equation*}
		% \label{eq:19}
